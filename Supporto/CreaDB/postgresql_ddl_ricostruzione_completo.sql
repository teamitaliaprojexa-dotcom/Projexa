...
WITH base AS (
    SELECT 
        ...
        a.project_id,
        b.cod_billing,
        SUM(a.offerta_hh)         AS offerta_hh,
        SUM(a.offerta_gg)         AS offerta_gg,
        SUM(a.cost_time_spent_hh) AS consuntivo_hh,
        SUM(a.cost_time_spent_gg) AS consuntivo_gg
    FROM proj_worker a
    JOIN proj_worker_cost b
        ON a.client_id = b.client_id
       AND a.worker_cost_id = b.id
    GROUP BY 
        a.tenant_id,
        a.user_id,
        a.client_id,
...
totali AS (
    SELECT
        ...
        client_id,
        project_id,
        SUM(offerta_hh) AS totale_offerta_hh,
        SUM(offerta_gg) AS totale_offerta_gg
    FROM base
    GROUP BY
        tenant_id,
        user_id,
...
sconti AS (
    SELECT
        ...
        /*
         * SCONTO GG
         * Stessa logica della tua proj_sconto:
         * (offerta_gg - valore3) / offerta_gg * 100
         */
        COALESCE(
            ROUND(
                (
                    (
                        COALESCE(t.totale_offerta_gg, 0)::numeric
                        - COALESCE(p.valore3, 0)::numeric
                    )
                    /
                    NULLIF(
                        COALESCE(t.totale_offerta_gg, 0)::numeric,
         ...
         * SCONTO HH
         * Stessa logica della tua proj_sconto:
         * (offerta_hh - valore3) / offerta_hh * 100
         */
        COALESCE(
            ROUND(
                (
                    (
                        COALESCE(t.totale_offerta_hh, 0)::numeric
                        - COALESCE(p.valore3, 0)::numeric
                    )
                    /
                    NULLIF(
    ...
    JOIN projects p
       ...
       AND t.project_id = p.argument::uuid
    WHERE p.campo::text = 'Importo'::text
),
...
base_scontata AS (
    SELECT
         ...
         */
        ROUND(
            b.offerta_hh::numeric
            * (
                1 - COALESCE(s.sconto_hh, 0)::numeric / 100
            ),
            0
        ) AS importo_scontato_hh,
         ...
         */
        ROUND(
            b.offerta_gg::numeric
            * (
                1 - COALESCE(s.sconto_gg, 0)::numeric / 100
            ),
            0
        ) AS importo_scontato_gg
...
unioned AS (
    ...
    SELECT 
        ...
        client_id,
        project_id,
        cod_billing::character varying(255) AS cod_billing,
        offerta_hh,
        offerta_gg,
        consuntivo_hh,
        consuntivo_gg,
    ...
    SELECT 
        ...
        client_id,
        project_id,
        'Totale'::character varying(255) AS cod_billing,

        SUM(offerta_hh)         AS offerta_hh,
        SUM(offerta_gg)         AS offerta_gg,
        SUM(consuntivo_hh)      AS consuntivo_hh,
        SUM(consuntivo_gg)      AS consuntivo_gg,
        SUM(importo_scontato_hh) AS importo_scontato_hh,
        SUM(importo_scontato_gg) AS importo_scontato_gg
...
ORDER BY 
    ...
    project_id,
    CASE 
        WHEN cod_billing = 'Totale' THEN 1
        ELSE 0
    END,
    cod_billing;
